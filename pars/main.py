...
def initiate():
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=str,
        help="One or more phone to search.",
    )
    parser.add_argument(
        "-i",
        "--ip",
    ...
    config.currentPhone = None
    config.currentIp = None
    config.phoneFoundInfo = None
    config.ipFoundInfo = None
...
if __name__ == "__main__":
        ...
        config.console.print(
            f"[white]{config.splash_line}[/white] | [red]by[/red] [blue]ch1brikess[/blue]"
        )
        ...
        if config.ai:
            config.console.print("[yellow1]:exclamation: By proceeding, you consent to share the found site names with Blackbird and ch1brikess AI for analysis.[/yellow1] [Y/n]", end="")
        ...
        if config.phone:
            for phone_number in config.phone:
                config.currentPhone = phone_number
                if config.dump or config.csv or config.pdf or config.json:
                    createSaveDirectory(config)
                ...
                try:
                    ...
                    if phonenumbers.is_valid_number(parse_number) and phonenumbers.is_possible_number(parse_number):
                        config.phoneFoundInfo = [{
                            ...
                            "timezones": ', '.join(timezone.time_zones_for_number(parse_number)),
                ...
                table = Table(title=f"[bold cyan]Phone Information: {phone_number}[/bold cyan]", show_header=False)
                table.add_column("Field", style="bold green")
                table.add_column("Value", style="white")

                phone_data = config.phoneFoundInfo[0]
                if phone_data.get("exists"):
                    table.add_row("Category", str(phone_data.get("category", "N/A")))
                    table.add_row("Exists", str(phone_data.get("exists", "N/A")))
                    table.add_row("Input", str(phone_data.get("input", "N/A")))
                    table.add_row("National_number", str(phone_data.get("national_number", "N/A")))
                    table.add_row("Carrier", str(phone_data.get("carrier", "N/A")))
                    table.add_row("Region", str(phone_data.get("region", "N/A")))
                    table.add_row("Timezones", str(phone_data.get("timezones", "N/A")))
                    table.add_row("Country_code", f"{phone_data.get('country_code')}, {phone_data.get('lon')}")
                else:
                    table.add_row("Error", phone_data.get("error", "Unknown"))

                config.console.print(table)

                if config.csv and config.phoneFoundInfo:
                    saveToCsv(config.phoneFoundInfo, config)
                if config.pdf and config.phoneFoundInfo:
                    saveToPdf(config.phoneFoundInfo, "phone", config)
                if config.json and config.phoneFoundInfo:
                    saveToJson(config.phoneFoundInfo, config)
        ...
        if config.ip:
            for ip in config.ip:
                ...
                try:
                    response = requests.get(url=f'http://ip-api.com/json/{ip}', timeout=config.timeout).json()
                    if response.get('status') == 'fail':
                        ...
                        try:
                            ...
                            map_filename = f"{response.get('query')}_{response.get('city')}.html"
                            map_path = os.path.join(config.saveDirectory, map_filename)
                            area.save(map_path)
                        except Exception as map_err:
                            print(f"[!] Failed to save map for {ip}: {map_err}")
                ...
                table = Table(title=f"[bold cyan]IP Information: {ip}[/bold cyan]", show_header=False)
                table.add_column("Field", style="bold green")
                table.add_column("Value", style="white")

                ip_data = config.ipFoundInfo[0]
                if ip_data.get("exists"):
                    table.add_row("IP", str(ip_data.get("ip", "N/A")))
                    table.add_row("Country", str(ip_data.get("country", "N/A")))
                    table.add_row("Region", str(ip_data.get("regionName", "N/A")))
                    table.add_row("City", str(ip_data.get("city", "N/A")))
                    table.add_row("ZIP", str(ip_data.get("zip", "N/A")))
                    table.add_row("ISP", str(ip_data.get("isp", "N/A")))
                    table.add_row("Organization", str(ip_data.get("org", "N/A")))
                    table.add_row("Coordinates", f"{ip_data.get('lat')}, {ip_data.get('lon')}")
                else:
                    table.add_row("Error", ip_data.get("error", "Unknown"))

                config.console.print(table)

                if config.csv and config.ipFoundInfo:
                    saveToCsv(config.ipFoundInfo, config)
                if config.pdf and config.ipFoundInfo:
                    saveToPdf(config.ipFoundInfo, "ip", config)
                if config.json and config.ipFoundInfo:
                    saveToJson(config.ipFoundInfo, config)
